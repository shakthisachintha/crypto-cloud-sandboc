...
export async function POST(req: Request) {
  ...
  try {
    const updated = (await updateActive(uuid, false)) as WalletRow;
    const result = {
    ...
    return NextResponse.json({ status: "error", result: { validation_error: `Failed to unsubscribe: ${msg}` } }, { status: 400 });
  }
}
